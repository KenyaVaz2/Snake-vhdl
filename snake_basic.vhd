...
entity snake_basic is
    Port (
        ...
        enter_raw     : in  STD_LOGIC;  
        arriba_raw    : in  STD_LOGIC;  
        izquierda_raw : in  STD_LOGIC;  
        derecha_raw   : in  STD_LOGIC;  
        abajo_raw     : in  STD_LOGIC;  
        filas         : out STD_LOGIC_VECTOR(7 downto 0);  
        columnas      : out STD_LOGIC_VECTOR(7 downto 0); 
        led_comida    : out STD_LOGIC; 
        seg7_catodos : out STD_LOGIC_VECTOR(6 downto 0);  
        seg7_anodos  : out STD_LOGIC_VECTOR(3 downto 0)   
    );
end snake_basic;
    ...
    begin
        if rising_edge(clk) then
            if reset_n = '0' then
                state <= WAIT_START;
                body_length <= 2;
                snake_body(0).x <= 3;  -- Cabeza inicio
                snake_body(0).y <= 3;
                snake_body(1).x <= 2;  -- Cola inicio
                snake_body(1).y <= 3;
                current_direction <= "10";  -- Dirección inicio: derecha
                comer <= '0';
                puntaje <= 0;
            else
                comer <= '0';  
